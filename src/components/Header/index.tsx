...
import ButtonLanguage from "@/components/ToogleLanguage"
const links = [
...
export default function Header(){
	...
	return (
		<header className={`fixed inset-x-0 top-0 z-20 border-b transition-all duration-300 ${scrolled ? "bg-black/80 backdrop-blur supports-backdrop-filter:bg-black/70" : "border-transparent bg-black/30 backdrop-blur supports-backdrop-filter:bg-black/20"}`}>
			<div className="pointer-events-auto mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
				<a href="/" className="flex items-center gap-3" onClick={close}>
					...
					<span className="text-(--seagle-color) font-semibold">Seagle</span>
				</a>
